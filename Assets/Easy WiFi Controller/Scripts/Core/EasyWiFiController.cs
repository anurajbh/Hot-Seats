...
namespace EasyWiFi.Core
{
    ...
    public class ScanResult : EventArgs
    {
    ...
    public static class EasyWiFiController
    {
        ...
        public static event OnScanInstructionReceivedHandler OnScanInstructionReceived;
        public delegate void OnScanInstructionReceivedHandler(ScanResult r);

        public static event ScanCommandHandler OnScanCommand;
        ...
        public static void sendScanInstruction() //sends a "scan" instruction to all client devices
        {
            Debug.Log("WIFI CONTROLLER: SENDING SCAN COMMAND");
            BaseControllerType temp;
            foreach (string client in serverSendKeys)
            {
                //bool send = true;
                string message = "PERFORM SCAN";
                ...
                //we don't want to send heartbeat to those marked as disconnected
                    ...
                    if (serverState != EasyWiFiConstants.CURRENT_SERVER_STATE.Disconnecting)
                    {
                        try
                        {
                            serverSendDictionary[client].Send(bytes, (int)bytes.Length, serverSendEndPointDictionary[client]);
                        }
                        catch (Exception ex)
                        {
            ...
            while (true)
            {
                try
                {
                    if (peerType.Equals(EasyWiFiConstants.PEERTYPE_SERVER))
                    {
                        ...
                        {
                            // this is the bluetooth message! 
                            //TODO
                            ScanResult r = new ScanResult();
                            r.result = message;
                            OnScanInstructionReceived(r);
                            Debug.Log("RESULT RECEIVED: " + message);
                        }
